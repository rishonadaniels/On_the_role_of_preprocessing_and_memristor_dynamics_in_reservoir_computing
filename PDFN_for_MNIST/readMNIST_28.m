...
%
% Usage:
% [imgs labels] = readMNIST_28(imgFile, labelFile, readDigits, offset)
%
% Parameters:
...
%
function [imgs labels] = readMNIST_28(imgFile, labelFile, readDigits, offset)


    % Read digits
    fid = fopen(imgFile, 'r', 'b');%% Open a file or obtain information about an open file
    header = fread(fid, 1, 'int32'); %% Read data from a binary file
    if header ~= 2051
    ...
    if offset > 0
        fseek(fid, w*h*offset, 'cof'); %% Move to specified position in file (cof--> current position in the file)
    end
